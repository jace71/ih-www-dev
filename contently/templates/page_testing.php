<table id="test-out"></table>

<style>
	#test-out tr td:first-child {
		font-weight: bold;
		vertical-align: top;
		padding-right: 20px;
	}

	#test-out tr td p {
		padding: 0;
		margin: 0;
	}

	#test-out tr td.warning {
		color: #c0a16b;
	}

	#test-out tr td.error {
		color: #b94a48;
	}

	#test-out tr td.success {
		color: #3c763d;
	}
</style>

<script>
	(function ($) {
		var table = $('#test-out'),
			tests = [
				{'title': 'Initial tests', data: {action: 'get_test_contently_initial_test'}},
				{
					'title': 'Summary',
					data: {action: 'get_contently_test_summary'},
					callback: function (template, response) {
						var result = [];
						result.push(['<strong>PHP version:</strong>', response.php_version].join(' '));
						result.push(['<strong>WP version:</strong>', response.wordpress_version].join(' '));
						result.push(['<strong>Plugin build:</strong>', response.build_version].join(' '));
						var plugins_list = ['<tr><td colspan="3">Used plugins:</td></tr>'];
						for (var i = 0; i < response.used_plugins.length; i++) {
							var plugin = response.used_plugins[i];
							plugins_list.push([
								'<tr>',
								'<td>', plugin.name, '</td>',
								'<td>', plugin.version, '</td>',
								'<td>', plugin.url, '</td>',
								'</tr>'
							].join(''));
						}
						result.push(['<table>', plugins_list.join(''), '</table>'].join(''));
						template.html(result.join('<br/>'));
					}
				},
				{'title': 'Test Connector Stream', data: {action: 'get_test_connector_stream'}},
				{'title': 'Test Connector Curl', data: {action: 'get_test_connector_curl'}},
				{
					'title': 'Current Connector',
					data: {action: 'get_test_connector_current'},
					callback: function (template, response) {
						template.html(response.transport);
					}
				},
				{
					'title': 'Test Filesystem Privileges',
					data: {action: 'get_test_filesystem_privileges'},
					callback: function (template, response) {
						template.html([
							['<p>', 'directory:', response.directory, ' - writable: ', response.is_writable_directory, '</p>'].join(''),
							['<p>', 'file:', response.file, ' - writable: ', response.is_writable_file, '</p>'].join('')
						].join(''));
					}
				},
				{'title': 'Test Logger Filesystem', data: {action: 'get_test_logger_filesystem'}},
				{
					'title': 'Check DB privileges',
					data: {action: 'get_test_database_privileges'},
					callback: function (template, response) {
						if (response.result) {
							var out = [];
							for (var i = 0; i < response.privileges.length; i++) {
								var privilege = response.privileges[i];
								out.push(['<p>', [privilege.Privilege, privilege.Context, privilege.Comment, privilege.result].join(' / '), '</p>'].join(''));
							}
							template.html(out.join(''));
						}
					}
				},
				{'title': 'Test Logger Database', data: {action: 'get_test_logger_database'}},
				{'title': 'Test Buffer', data: {action: 'get_contently_test_buffer'}}
			];

		function sendTestRequest(test) {
			var render_template = $('.result.' + test.data.action);

			return $.ajax({
				url: '?contently_pull=<?php echo $hash_key; ?>=&type=test&action=' + test.data.action,
				type: 'GET'
			}).done(function (response) {
				var data;

				try {
					data = JSON.parse(response);
				} catch (e) {
					render_template.html('error ' + e.text);
				}

				if (typeof data !== 'object') {
					render_template.addClass('error');
					render_template.html('error' + ' [' + response + ']');
				} else {
					if (data.result === true)
						render_template.addClass('success');
					else
						render_template.addClass('warning');

					render_template.html(data.result);

					if (test.callback) {
						test.callback(render_template, data);
					}
				}
			}).fail(function (jqXHR, textStatus, errorThrown) {
				render_template.addClass('error');
				try {
					render_template.html('error ' + JSON.stringify(jqXHR));
				} catch (e) {
					render_template.html('error ' + jqXHR);
				}
			});
		}

		function runTest(index) {
			if (index < tests.length) {
				sendTestRequest(tests[index])
					.always(function () {
						runTest(index + 1);
					});
			}
		}

		var i, test;
		for (i = 0; i < tests.length; i++) {
			test = tests[i];
			var template = $('<tr>\n    <td class="title"></td>\n    <td class="result"><img src="data:image/gif;base64," height="10px" width="10px"></td>\n</tr>');
			template.find('.title').html(test.title);
			template.find('.result').addClass(test.data.action);
			table.append(template);
		}

		runTest(0);

	})(jQuery);
</script>